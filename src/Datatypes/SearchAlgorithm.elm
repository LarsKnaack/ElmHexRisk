module Datatypes.SearchAlgorithm exposing (..)

import Array exposing (Array)
import Datatypes.Board exposing (Cell)
import Datatypes.CubeCoordinates exposing (CubeCoordinates, cubeToIndex)
...
breadthFirst start end visited toVisit cells =
    let
        neighbours = validNeighbours start cells
    in
...
validNeighbours start cells =
    let
        allNeighbours = List.map (\dir -> Array.get (cubeToIndex (CubeCoordinates (start.position.x + dir.x) (start.position.y + dir.y) (start.position.z + dir.z))) cells) neighbourDirections
    in
    List.foldl (
    \cell acc ->
    case cell of
        Nothing -> acc
        Just c -> if c.occupant == start.occupant then c :: acc else acc
    ) [] allNeighbours
